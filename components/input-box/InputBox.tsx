...
import { convertNumToAlpha } from "../../helper/number-alpha"
import { realNumber } from "../../helper/real-number"
...
export const InputBox: React.FC<props> = (props, ref) => {
  const {
    control,
    name,
    rules,
    ...
    <div className={classNames(`flex flex-col items-stretch`, mb, containerClassName)}>
      ...
      >
        ...
        <label className={classNames(`inp-wrapper w-full rounded-lg flex flex-row justify-center `, labelStype)}>
          ...
          {!hideCurrenctyTag && isCurrency && <span className="tooltip whitespace-nowrap z-60">{convertNumToAlpha(realNumber(field.value))} تومان</span>}

          <span className="inp-label text-xs select-none">{rest.placeholder}</span>
          {(isCurrency || isNumric) && !hideCurrenctyTag && (
            <span className="inline-flex items-center px-2 select-none bg-gray-50 dark:bg-gray-600 text-xs rounded-l-lg">{currencyTag}</span>
          )}
        </label>
      </div>
